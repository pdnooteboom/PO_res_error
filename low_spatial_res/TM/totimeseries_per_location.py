...
#%%
print dirRead + 'sp%d_dd%d/'%(int(sp),int(dd)) + 'grid_dd10_sp6_res1.nc'  # + 'concatenated_sp%d_dd%d_res%d.nc'%(int(sp),int(dd),res)
pfile = Dataset(dirRead + 'sp%d_dd%d/'%(int(sp),int(dd)) + 'concatenated_smagorinski_Cs%.1f_sp%d_dd%d_res%d.nc'%(Cs,sp,dd,res))
# + 'concatenated_sp%d_dd%d_res%d.nc'%(int(sp),int(dd),res) )
pfilefix = Dataset(dirRead + 'surface/' + 'surfacegrid_id_dd10_res1.nc')
...
print 'dirWrite:   ',dirWrite
dataset = Dataset(dirWrite  + 'timeseries_per_location_smagorinksi_Cs%.1f_ddeg%d_sp%d_dd%d.nc'%(Cs,ddeg, int(sp),int(dd)),'w',format='NETCDF4_CLASSIC')
